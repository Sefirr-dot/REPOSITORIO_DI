...
public class Cola {
	...
	static Queue<Persona> colaPersonas = new LinkedList<Persona>();
	...
	private static void crearCola(Queue<Persona> colaPersonas, int numPersonasCola) {
		for (int i = 0; i < numPersonasCola; i++) {
			colaPersonas.add(generarPersona());
		
	}
}
	private static void mostratTotal(Queue<Persona> colaPersonas, int numPersonasCola) {
		double total = 0;
		for (int i = 0; i < numPersonasCola; i++) {
			int edad= colaPersonas.poll().getEdad();
			if(edad<11) {
